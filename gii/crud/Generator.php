...
class Generator extends \yii\gii\Generator
{
    ...
    /**
     * Get the base class for views.
     * >>> Code by ASi
     */
    public function getViewBaseClass() 
    {
    	$firstNsPart = substr($this->controllerClass, 0, strpos($this->controllerClass, '\\'));

   		switch ($firstNsPart) {
   			case 'frontend':
   				return 'asinfotrack\components\FrontendView';
   			case 'backend':
   				return 'asinfotrack\components\BackendView';
   			case 'common':
   			default:
   				return 'asinfotrack\components\View';
   		}
    }
        ...
        foreach (scandir($templatePath) as $file) {
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                if (substr($file, 0, 1) == '_') {
                	$targetPath = $viewPath . '/partials/' . $file;
                } else {
                	$targetPath = $viewPath . '/' . $file;
                }
            	$files[] = new CodeFile($targetPath, $this->render("views/$file"));
            }
        }
